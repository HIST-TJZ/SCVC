# CP² Killing矢量归一化积分的Explicit Calculation
...
等等！我之前Derivation时得到的是(1/3 + 1/12) = 5/12？不对...
...
这样：
∫(−Z†X²Z) = −(Vol/3)·Tr(X²)
∫(−(Z†XZ)²) = −(Vol/12)·Tr(X²)   [类似(5)的Derivation]
...
好！这与之前的MCResult一致。之前的代码里 also 用的是 −Z†X²Z + |Z†XZ|²，但那是错的——应该是 −Z†X²Z − (Z†XZ)²。

但代码中的 `killing_norm_sq` 函数直接计算 |K^H|² = |iTZ − (Z†iTZ)Z|²，这是直接用定义计算的，所以MCResult是对的。而我在Derivation中把公式写成了 −Z†X²Z + |Z†XZ|² 是错误的——应该是 −Z†X²Z − (Z†XZ)²。

不过因为(Z†XZ)² = −|Z†XZ|²，所以 −Z†X²Z − (Z†XZ)² = −Z†X²Z + |Z†XZ|²。也就是说，如果在Derivation中不小心把−(Z†XZ)²写成了+|Z†XZ|²，Result其实是正确的（巧合）。但正确的表达式是：
...
- MC采样确认I^{aa} = π²/16 ≈ 0.616850（200k样本，Precision~0.1%）
- 解析Result严格吻合
...
标准模型中的规范耦合归一化高度吻合，Deviation仅~0.93%——这来自
