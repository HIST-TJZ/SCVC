...
func _input(event):
	if event is InputEventMouseButton:
		match event.button_index:
			...
			MOUSE_BUTTON_WHEEL_UP:
				distance = max(min_distance, distance - zoom_speed * (distance * 0.08))
				_update_camera()
			MOUSE_BUTTON_WHEEL_DOWN:
				distance = min(max_distance, distance + zoom_speed * (distance * 0.08))
				_update_camera()
	...
	if event is InputEventMouseMotion:
		if orbit_active:
			...
			pivot -= right * delta.x * pan_speed * (distance * 0.08) - up * delta.y * pan_speed * (distance * 0.08)
			last_mouse = event.position
			_update_camera()
...
func _process(_delta):
	...
	var move_speed: float = distance * 0.66 * _delta  # scales with zoom: close=slow, far=fast
