...
const VFMCompute = preload("res://scripts/vfm_compute.gd")
const GPLUT = preload("res://scripts/gp_lut.gd")
...
# E2 Phase 1: Electron-scale parameters (mass rescaling, SCVC-derived)
# Only xi scales with mass: xi_e = xi_p * sqrt(m_p/m_e) = 0.25 * 42.85 = 10.71
# G_EM = 2.00 is UNIVERSAL (same coupling for all particles, per GP AI verification)
# alpha = 1/137 is SCVC OUTPUT: (4*pi^3 + pi^2 + pi)^-1, not an input
const XI_ELECTRON: float = 10.71         # Electron vortex core radius (lighter -> more spread)
const MF_RATIO_EP: float = 1836.0        # m_p/m_e (from SCVC R1-R4)
...
# =====================================================================
# SCVC QUARK SIMULATION ? CAPABILITY BOUNDARY
# =====================================================================
# WHAT THIS SIMULATION ACTUALLY IMPLEMENTS (2026-07-24):
#
# LAYER 1: Quark Confinement (FULLY IMPLEMENTED)
#   - Color SU(3) winding numbers (w_c1, w_c2) on point vortices
#   - G_STRONG=3.30 logarithmic confinement: F ~ color_dot * r/(xi^2+r^2)
#   - Color singlets (R+G+B=0) emerge automatically from force balance
#   - Quark-quark EM: G_EM=2.00, charge = w_w + w_y
#   - Vacuum stability: E_CORE=2.1322 from GP equation
#
# LAYER 2: Nucleon Formation (EMERGENT)
#   - Protons (uud) and neutrons (udd) self-assemble from 3-quark clusters
#   - Color singlet + isospin (|sum w_w|~0.5) identification algorithm
#   - Observed: C-12 alpha clusters, Li-7 alpha+triton, He-4 dual configs
#   - Spring forces maintain intra-nucleon quark binding
#
# LAYER 3: Nucleon-Nucleon Forces (N1+N2, IMPLEMENTED)
#   - N1: OPEP attraction (pi exchange) + hard-core repulsion (omega)
#   - N2: Three-body force (GP medium, sat-calibrated)
#   - N2: Spin-orbit force (OPEP LS, L.S approximated)
#   - All constants SCVC-derived, zero free parameters (except C_3N sat-cal)
#   - VFM Biot-Savart: CPU, every 4 frames, quark rings only
#
# WHERE EMERGENCE STOPS (NOT YET IMPLEMENTED):
#   - Nuclear shell structure (magic numbers: 2,8,20,28,50...)
#     Requires: full spin-orbit coupling -> LS splitting of shells
#   - Quantitative binding energies
#     Requires: N3-N4 multi-body, proper tensor force, full spin tracking
#   - Beta decay of bound neutrons
#     Requires: weak interaction for n->p+e+nu in nuclear medium
#   - Heavy nuclei (A > ~20)
#     Requires: shell model or DFT, beyond N-body classical simulation
#   - Nuclear reactions / cross sections
#     Requires: quantum tunneling, not classical trajectories
#
# WHAT YOU SEE IS:
#   - Quarks -> nucleons: GENUINE EMERGENCE (color force -> clusters)
#   - Nucleons -> light nuclei: QUALITATIVE (structures look right, no MeV precision)
#   - Nuclei -> shell structure: NOT YET (needs LS splitting)
#   - Anything beyond A~20: PURELY HEURISTIC (SCVC-inspired, not SCVC-derived)
#
# SCVC HONESTY LEVELS:
#   [SCVC-LOCKED]  = pure SCVC derivation, zero freedom
#   [SAT-CAL]      = SCVC functional form, normalization from nuclear saturation
#   [STANDARD-NP]  = standard nuclear physics adapted to SCVC units
#   [EMPIRICAL]    = experimental input (e.g. CKM matrix V_ud)
#   [HEURISTIC]    = visually inspired, not SCVC-derived
# =====================================================================
...
# ===== N2: Multi-Body Forces (SCVC-derived, 2026-07-24) =====
# Reference: N2_MultiBody_SCVC_Derivation_results.md
...
const N2_ENABLE_TENSOR: bool = false      # Tensor force: VFM partially covers; explicit adds ~5% correction       # Numerical guard: cap per-cluster force (SCVC HONESTY: hard core
...
var quark_visual_scale: float = 0.20  # visual scale of quark spheres (-/=)
...
func _ready():
	# Route A: GPU VFM compute DISABLED (D3D12 conflicts with Godot renderer)
	# If GPU is ever re-enabled, VFMCompute.instance().initialize() must run on a
	# separate thread with its own D3D12 device, not during _ready().
	_vfm_use_gpu = false
	_vfm_gpu_ready = false
	print_rich("[color=yellow]VFM GPU disabled (D3D12 conflict), CPU fallback active[/color]")
	
	# E2 Phase 2: Load GP lookup tables (graceful fallback if not yet computed)
	GPLUT.instance().load_tables()
...
func _process(delta: float):
	...
	if _nn_enabled:
		_nn_recompute_tick -= 1
		if _nn_recompute_tick <= 0:
	...
	if vfm_enabled and _vfm_recompute_tick <= 0:
		_vfm_recompute_tick = VFM_RECOMPUTE_EVERY
		# CPU recompute: tag rings to recompute this frame
			# CPU recompute: tag rings to recompute this frame
		for ring in rings:
			if is_instance_valid(ring) and ring.active:
				ring.set_meta("_vfm_needs_recompute", true)
	# ================================================================
	# RING UPDATE: Electron vs Non-Electron (SCVC Clean Separation)
	# ================================================================
	# ELECTRONS (charge != 0): SCVC constrained orbit ONLY.
	#   kappa = h/m_e -> omega = hbar/(m_e * r^2).
	#   No VFM. No free dynamics. Topological quantization, not force balance.
	# NON-ELECTRONS (mesons etc.): VFM Biot-Savart (if enabled) or free dynamics.
	# ================================================================
	for ring in rings:
		if not is_instance_valid(ring) or not ring.active or ring.is_dragging:
			continue
		
		var is_electron: bool = ring.charge() != 0.0
		
		if is_electron:
			# SCVC CONSTRAINED ORBIT: electron position from topological quantization.
			# The orbit center tracks the nucleus COM. Radius and angular velocity
			# are SCVC-locked (kappa = h/m_e). No classical force balance assumed.
			ring.physics_update(phys_delta, [])
			
			# GP LUT: quantum pressure correction to orbit center (E2 Phase 2)
			# Small perturbation on top of constrained orbit, not primary dynamics.
			_apply_gp_lut_to_electron(ring)
		else:
			# Non-electron ring (meson, etc.): VFM or constrained
			if vfm_enabled and ring.target_orbit_radius <= 0.0:
				# VFM Biot-Savart path (CPU, decimated every N frames)
				var vfm_vels: Array
				if ring.get_meta('_vfm_needs_recompute', false):
					ring.set_meta('_vfm_needs_recompute', false)
					vfm_vels = _compute_vfm_velocities(ring)
					ring.set_meta('_vfm_cached_vels', vfm_vels)
				else:
					vfm_vels = ring.get_meta('_vfm_cached_vels', [])
				_compute_ring_forces(ring)
				var gauge_forces: Array = ring.get_meta('_ext_forces', [])
				var merged_vels: Array = []
				for si in range(ring.num_segments):
					var v_total: Vector3 = Vector3.ZERO
					if si < vfm_vels.size():
						v_total += vfm_vels[si]
					if si < gauge_forces.size():
						v_total += gauge_forces[si] * delta / max(ring.mass_factor, 0.01)
					merged_vels.append(v_total)
				ring.set_meta('_vfm_velocities', merged_vels)
				ring.physics_update_vfm(phys_delta)
			else:
				# Constrained or free (non-VFM)
...
func create_vortex(pos: Vector3, wc1: float, wc2: float, ww: float, wy: float, vel: Vector3 = Vector3.ZERO, mf: float = 0.0) -> Node3D:
	# SCVC 3D: add tiny Y jitter to break planar symmetry (all forces are 3D, init must be too)
	if abs(pos.y) < 0.001:
		pos.y += randf_range(-0.15, 0.15)
...
# E2 Phase 2: GP quantum pressure correction for electron constrained orbits.
# Applies a small center-of-mass force derived from offline GP solutions.
# This is a PERTURBATION on the SCVC constrained orbit, not primary dynamics.
func _apply_gp_lut_to_electron(ring: Node3D):
	if not GPLUT.instance().has_table(GPLUT.PairType.EP):
		return
	if _cluster_cache_frame != Engine.get_process_frames():
		return  # stale cluster cache, skip this frame
	for cl in _cluster_cache:
		if cl.size() < 3:
			continue
		...
		if not valid:
			continue
		com /= float(cl.size())
		var r_center: float = ring.position.distance_to(com)
		if r_center < GPLUT.R_MAX and r_center > 0.01:
			var dvdr: float = GPLUT.instance().get_force_derivative(r_center, GPLUT.PairType.EP)
			var dir_center: Vector3 = (com - ring.position).normalized()
			# F = -dV/dr, applied as small correction to orbit center
			var f_center: Vector3 = dir_center * (-dvdr)
			# Apply to ring center velocity (constrained orbit perturbation)
			ring.center_velocity += f_center * 0.0167 / max(ring.mass_factor, 0.01)  # ~1 frame worth
			ring.position += ring.center_velocity * 0.0167  # small nudge
...
# =====================================================================
# Route A: GPU VFM Dispatch (collects all ring data, dispatches once)
# =====================================================================
func _collect_vfm_ring_data() -> Dictionary:
	# Returns {seg_pos, seg_map, ring_off, ring_rad, vor_pos, ring_order}
	# ring_order: Array of rings in order they appear in seg_map
	var seg_pos: PackedFloat32Array = PackedFloat32Array()
	var seg_map: PackedInt32Array = PackedInt32Array()
	var ring_off: PackedInt32Array = PackedInt32Array()
	var ring_rad: PackedFloat32Array = PackedFloat32Array()
	var ring_order: Array = []
	
	var offset: int = 0
	for ring in rings:
		if not is_instance_valid(ring) or not ring.active: continue
		if ring.target_orbit_radius > 0.0: continue  # constrained rings skip VFM
		ring_off.append(offset)
		ring_rad.append(ring.radius)
		ring_order.append(ring)
		var n_seg: int = ring.num_segments
		for i in range(n_seg):
			var pos: Vector3 = ring.get_segment_world_pos(i)
			seg_pos.append(pos.x); seg_pos.append(pos.y); seg_pos.append(pos.z)
			seg_map.append(ring_order.size() - 1)
		offset += n_seg
	
	var vor_pos: PackedFloat32Array = PackedFloat32Array()
	for v in vortices:
		if not is_instance_valid(v) or not v.visible: continue
		vor_pos.append(v.position.x); vor_pos.append(v.position.y); vor_pos.append(v.position.z)
	
	return {"seg_pos": seg_pos, "seg_map": seg_map, "ring_off": ring_off,
			"ring_rad": ring_rad, "vor_pos": vor_pos, "ring_order": ring_order}

# GPU path removed (D3D12 conflict). Function kept for reference.
func _dispatch_vfm_gpu():  # DEPRECATED
	var data: Dictionary = _collect_vfm_ring_data()
	var n_seg: int = data["seg_pos"].size() / 3
	var n_ring: int = data["ring_order"].size()
	var n_vor: int = data["vor_pos"].size() / 3
	
	if n_seg == 0 or n_ring == 0: return
	
	var gpu = VFMCompute.instance()
	var out_vels: PackedFloat32Array = gpu.compute_all(
		data["seg_pos"], data["seg_map"], data["ring_off"],
		data["ring_rad"], data["vor_pos"], n_seg, n_ring, n_vor
	)
	
	if out_vels.size() == 0: return
	
	# Distribute velocities back to rings
	var offset: int = 0
	for ring_idx in range(n_ring):
		var ring = data["ring_order"][ring_idx]
		if not is_instance_valid(ring): continue
		var n: int = ring.num_segments
		var vels: Array = []
		for i in range(n):
			var idx: int = (offset + i) * 3
			if idx + 2 < out_vels.size():
				vels.append(Vector3(out_vels[idx], out_vels[idx + 1], out_vels[idx + 2]))
			else:
				vels.append(Vector3.ZERO)
		ring.set_meta("_vfm_gpu_vels", vels)
		offset += n
...
# Step 1: Identify nucleons from quark windings (expensive, runs every NN_RECOMPUTE_EVERY frames)
func _identify_nucleons() -> Array:
	var out: Array = []
	var quarks: Array = []
	for v in vortices:
		if not is_instance_valid(v) or not v.visible: continue
		var cmag: float = sqrt(v.w_c1 * v.w_c1 + v.w_c2 * v.w_c2)
		if cmag < 0.05: continue
		quarks.append(v)
	...
	for cl in spatial_clusters:
		var remaining: Array = cl.duplicate()
		while remaining.size() >= 3:
			...
			if best_trio.size() == 3 and best_score < (COLOR_EPSILON + ISOSPIN_EPSILON):
				...
				var com: Vector3 = (a.position + b.position + c.position) / 3.0
				out.append({"com": com, "quarks": [a, b, c]})
...
# Step 2: Apply NN forces between identified nucleons (lightweight, runs every frame)
# N2: Three-body force from GP medium-induced vortex interaction.
# Formula: V_3N = -C_3N * exp(-(r12+r23+r31)/R_CUT) / (r12 * r23 * r31)
# SCVC HONESTY: Coefficient C_3N=0.1183 calibrated to nuclear saturation density.
# Functional form is GP-derived; normalization is semi-empirical. Confidence: 75%.
func _apply_three_body_forces(nucleons: Array):
	if nucleons.size() < 3 or not N2_ENABLE_3N:
		return
	for i in range(nucleons.size()):
		for j in range(i + 1, nucleons.size()):
			for k in range(j + 1, nucleons.size()):
				var ni = nucleons[i]; var nj = nucleons[j]; var nk = nucleons[k]
				var r12: float = ni["com"].distance_to(nj["com"])
				var r23: float = nj["com"].distance_to(nk["com"])
				var r31: float = nk["com"].distance_to(ni["com"])
				var r_sum: float = r12 + r23 + r31
				if r_sum < 0.001: continue
				var v3: float = -N2_C_3N * exp(-r_sum / N2_R_CUT_3N) / max(r12 * r23 * r31, 0.0001)
				var dv_dr12: float = v3 * (1.0 / N2_R_CUT_3N + 1.0 / max(r12, 0.001))
				var dv_dr23: float = v3 * (1.0 / N2_R_CUT_3N + 1.0 / max(r23, 0.001))
				var dv_dr31: float = v3 * (1.0 / N2_R_CUT_3N + 1.0 / max(r31, 0.001))
				var dir12: Vector3 = (nj["com"] - ni["com"]).normalized() if r12 > 0.001 else Vector3.ZERO
				var dir23: Vector3 = (nk["com"] - nj["com"]).normalized() if r23 > 0.001 else Vector3.ZERO
				var dir31: Vector3 = (ni["com"] - nk["com"]).normalized() if r31 > 0.001 else Vector3.ZERO
...
# SCVC HONESTY: OPEP LS is standard nuclear physics; Magnus correction is ~3%.
# L.S approximated from nucleon velocities (full spin tracking not yet implemented).
# Confidence: 85%.
func _apply_spin_orbit_forces(nucleons: Array):
	if nucleons.size() < 2 or not N2_ENABLE_LS:
		return
	for i in range(nucleons.size()):
		var ni = nucleons[i]
		if not ni.has("velocity"): continue
		for j in range(i + 1, nucleons.size()):
			var nj = nucleons[j]
			if not nj.has("velocity"): continue
			var r_vec: Vector3 = nj["com"] - ni["com"]
			var r: float = r_vec.length()
			if r < 0.001: continue
			var v_rel: Vector3 = nj["velocity"] - ni["velocity"]
			var L: Vector3 = r_vec.cross(v_rel)
			var L_dot_S: float = L.length() * 0.5
			var x: float = NN_M_PI * r
			if x < 0.01: continue
			var g_ls: float = (3.0/x + 3.0/(x*x)) * exp(-x) / x
			var f_ls: float = N2_C_LS * L_dot_S * g_ls
			f_ls = min(f_ls, NN_FORCE_CAP)
			var dir: Vector3 = r_vec.normalized()
			var f_perp: Vector3 = L.cross(dir).normalized() * f_ls * 0.1
			_apply_force_to_nucleon(ni, -f_perp)
			_apply_force_to_nucleon(nj, f_perp)

# Helper: distribute force across all quarks in a nucleon
func _apply_force_to_nucleon(nuc: Dictionary, force: Vector3):
	if not nuc.has("quarks"): return
	var qs: Array = nuc["quarks"]
	if qs.size() == 0: return
	var f_per_quark: Vector3 = force / float(qs.size())
	for q in qs:
		if is_instance_valid(q):
			q.force_accum += f_per_quark

func _apply_nn_forces(nucleons: Array):
	if nucleons.size() < 2: return
	
	# N2: Three-body force (GP medium-induced, sat-calibrated)
	_apply_three_body_forces(nucleons)
	
	# Track nucleon velocities for N2 spin-orbit (store in dict)
	for nuc in nucleons:
		if nuc.has("_prev_com") and nuc.has("com"):
			nuc["velocity"] = (nuc["com"] - nuc["_prev_com"]) / 0.0167  # ~60fps delta
		nuc["_prev_com"] = nuc["com"]
	
	# N1: Two-body central potential (OPEP + hard core repulsion)
	for i in range(nucleons.size()):
		var ni = nucleons[i]
		for j in range(i + 1, nucleons.size()):
			var nj = nucleons[j]
			...
			var force: Vector3 = dir * f_mag
			_apply_force_to_nucleon(ni, force)
			_apply_force_to_nucleon(nj, -force)
	
	# N2: Spin-orbit force (OPEP LS + SCVC Magnus)
	_apply_spin_orbit_forces(nucleons)
