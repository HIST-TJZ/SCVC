...
var raw_mode: bool = false  # R key: strip all constraints + VFM, bare EM+spring dynamics
...
func _update_status():
	...
	if raw_mode:
		lines.append(_tr('RAW [R]: VFM off (electrons stay constrained)', 'RAW [R]: VFM关 (电子保持约束)'))
...
func _toggle_raw_mode():
	raw_mode = not raw_mode
	if physics:
		physics.vfm_enabled = not raw_mode
	if raw_mode:
		# SCVC HONESTY: Raw mode disables VFM for non-electron rings only.
		# Electrons ALWAYS use SCVC constrained orbits (kappa = h/m_e).
		# Stripping electron constraints = classical Rutherford problem = non-physical.
		status_label.text = _tr('RAW [R]: VFM off (quark rings only). Electrons stay constrained.', 'RAW [R]: VFM关 (仅夸克环)。电子保持约束。')
	else:
		status_label.text = _tr('VFM restored', 'VFM已恢复')
	_update_status()
...
func _scene_element(Z: int):
	...
	var remaining: int = Z
	var shell_plan: Array = []
	var n_electrons: Dictionary = {}
	...
	for sp in shell_plan:
		var n_sp: int = sp["n"]
		var n_inner_sp: int = sp["n_inner"]
		var n_same_sp: int = sp["n_same"]
		var count_sp: int = sp["count"]
		for i in range(count_sp):
			var angle: float = (float(i) / count_sp) * 360.0
			var zeff: float = _compute_zeff(Z, n_sp, n_inner_sp, n_same_sp + i)
			_place_electron_ring(Vector3.ZERO, n_sp, angle, Z, 400.0, zeff)

	# Status label
	var verif: String = "SCVC-locked" if Z <= 10 else ("heuristic" if Z <= 18 else "HEURISTIC")
	var emoji: String = "SCVC" if Z <= 10 else ("heuristic" if Z <= 18 else "HEURISTIC")
...
## SCVC HONESTY: demo -- VFM quark dynamics test. Electrons stay SCVC-constrained.
## Two protons with VFM quark dynamics. Electrons at SCVC Bohr orbits.
## VFM Biot-Savart drives quark ring dynamics. Electron orbits are SCVC-locked.
## If covalent bond forms: rings move to bond region, protons settle.
	_center_all_nuclei()
# H2 VFM Test: 2 protons (VFM quark dynamics) + 2 SCVC-constrained electrons
func _scene_h2_vfm():
	# SCVC HONESTY: VFM tests quark/proton dynamics. Electrons remain SCVC-constrained.
	# Two protons with VFM-enabled quark rings, electrons at Bohr orbits.
