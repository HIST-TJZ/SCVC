...
func _build_ring():
	...
	for i in range(num_segments):
		...
		if batch_render:
			# GPU batching: use lightweight Node3D placeholders, MultiMesh handles rendering
			var seg = Node3D.new()
			seg.name = "Seg%d" % i
			...
			# Individual render: full MeshInstance3D (debug/legacy)
	...
	# Spring connections between segments
	for i in range(num_segments):
		var j = (i + 1) % num_segments
		if batch_render:
			# Lightweight placeholder, MultiMesh handles rendering
			var sp = Node3D.new()
	...
	# Normal indicator (small arrow) - lightweight, only created when batch_render is OFF
	if not batch_render:
...
func _safe_look_at(node: Node3D, target: Vector3, up: Vector3 = Vector3.UP):
	if not is_instance_valid(node):
		return
	# NaN guard: check all inputs
	if is_nan(target.x) or is_nan(target.y) or is_nan(target.z):
		return
	var gp := node.global_position
	if is_nan(gp.x) or is_nan(gp.y) or is_nan(gp.z):
		return
	var to_target := target - gp
	if to_target.length_squared() < 0.000001:
		return
	var dir := to_target.normalized()
	if is_nan(dir.x) or is_nan(dir.y) or is_nan(dir.z):
		return
	# Direct basis construction: avoids all look_at colinear/zero warnings
	...
	if right.length_squared() < 0.0001:
		return  # degenerate, can't orient
	right = right.normalized()
	var basis_up := right.cross(dir).normalized()
	node.transform.basis = Basis(right, dir, basis_up)
...
func _update_springs():
	for i in range(num_segments):
		...
		if is_nan(ap.x) or is_nan(ap.y) or is_nan(ap.z): continue
		if is_nan(bp.x) or is_nan(bp.y) or is_nan(bp.z): continue
		var mid = (ap + bp) * 0.5
		var dist = ap.distance_to(bp)
		if is_nan(mid.x) or is_nan(mid.y) or is_nan(mid.z): continue
		sp.position = mid
		if dist > 0.01 and not is_nan(dist):
			_safe_look_at(sp, b.global_position, Vector3.UP)
			var scl = Vector3(1, dist, 1)
			if not is_nan(scl.x) and not is_nan(scl.y) and not is_nan(scl.z):
				sp.scale = scl

	if _arrow_node and is_instance_valid(_arrow_node):
		var ap := normal * (radius + 0.3)
		if not is_nan(ap.x) and not is_nan(ap.y) and not is_nan(ap.z):
			_arrow_node.position = ap
		var gp := global_position
		if not is_nan(gp.x) and not is_nan(gp.y) and not is_nan(gp.z):
			_safe_look_at(_arrow_node, gp + normal * 2.0, Vector3.UP)
...
func physics_update(delta: float, external_forces_per_segment: Array):
	...
	# ===== BRANCH 1: SCVC CONSTRAINED ORBIT =====
	# Electrons: kappa = h/m_e topological quantization.
	# Orbit radius and angular velocity are SCVC-locked, NOT from classical force balance.
	if target_orbit_radius > 0.0:
		_physics_constrained(delta)
		return

	# ===== BRANCH 2: FREE DYNAMICS (non-electron rings only: mesons, etc.) =====
	# SCVC HONESTY: This path is for non-electron vortex rings.
	# Electrons MUST use constrained orbits; classical free dynamics cannot form atoms.
	...
	for i in range(num_segments):
		...
		if is_nan(_segment_velocities[i].x) or is_nan(_segment_velocities[i].y) or is_nan(_segment_velocities[i].z):
	...
	if not is_nan(new_center.x) and not is_nan(new_center.y) and not is_nan(new_center.z):
		position += new_center
...
## SCVC CONSTRAINED ORBIT: Electron ring dynamics from topological quantization.
## kappa = h/m_e -> omega = hbar/(m_e * r^2). No classical force balance.
## Ring center tracks nucleus COM. Radius and omega are SCVC-locked.
## Springs maintain ring shape; no external forces needed.
...
func physics_update_vfm(delta: float):
	...
	for i in range(num_segments):
		...
		if is_nan(spring_force.x) or is_nan(spring_force.y) or is_nan(spring_force.z):
