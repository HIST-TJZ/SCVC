...
| α⁻¹ 三项结构 | ~90% | DH 局域化 + 等变上同调 |
| 费米子质量比（9/9） | ~85-90% | 离散扫描唯一解 + 群论因子 |
| sin²θ_W = 1/4 → 0.2300 | ~85% | 树级几何 + RG 修正 |
| α_s⁻¹ = 16π | ~90% | CP² DH 求和 |
...
| PMNS | ~60% | θ₁₂ 缺口 −92% |
| 暗物质不存在（预言） | 可证伪 | 等实验判决 |
| **综合** | **~85-90%** | 核心链90%+，费米子离散唯一85-90%，预言待验 |
...
逻辑链中的已知缺口和各环节置信度详见下方"已知理论张力"和上方"综合置信度"表。


这四个问题不推翻框架，但代表当前推导链中尚未完全闭合的结构性缺口。主动列在这里，比被外部审计打出来体面。

| # | 张力 | 性质 | 影响 | 当前状态 |
|:--:|:---|:---|:---|:---|
| T1 | **CP² spin^c + k=1 的选择** | 后验选择 | 代际数目 | CP² w₂≠0，非 spin 流形。承载费米子需 spin^c 结构 + U(1) 线丛。通量 k=1 的选择（而非 k=2,3,...）是"最小非平凡通量"论证，非强制推导。若 k≠1，Atiyah-Singer 指数 ≠3。 |
| T2 | **截锥模空间未显式构造** | 存在性未证 | α⁻¹ 三项结构 | DH 求和 = 4π³+π²+π 的前提：涡旋环模空间确为 toric Kähler 3-fold、矩多面体确为截锥、不动点恰好 3 个。GKM 图已绕过部分验证，但流形存在性未独立证明（~5% 残差）。 |
| T3 | **(2π) 归一化消去是论证** | 论证非证明 | α⁻¹ 数值 | Q-exact 局域化路径积分测度含 (2π)^(-n) 因子。Π(0) ∝ (2π)^(-n)×Z。框架用"三层吸收机制"论证因子恰好消去——这是论证（argument），非严格证明（proof）。若消不去，α⁻¹≠4π³+π²+π。 |
| T4 | **7D/8D 维度计数** | 定义问题 | 总维数 | 框架声称 7D 时空（4D+M_vac 3D），但 Atiyah-Singer、DH 求和、费米子零模均在 CP²（4D）上计算。CP² 承载物理算符却不计入物理维度，靠 BEC 序参量两相衔接（体相 7D→核心相局域 8D）解释，但数学上尚未严格形式化。 |

**这四条是框架目前最大的不确定性来源，合计贡献约 14% 的综合残差。** 闭合任何一条都会显著提升整体置信度。其中 T2（截锥模空间）最关键——它直接决定 α⁻¹ 三项结构的数学基础是否成立。
